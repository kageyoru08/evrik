...
All 90 protected input-file checks across the 18 trials passed. The literature artifacts consistently distinguished the contradictory widget results and abstract-only evidence. Detailed semantic review and any post-clarification cohort are reported separately in the release notes and verification artifacts. Complete per-tool transcripts and per-trial token usage are unavailable for these native subagents. Exact skill reads, training-only access chronology, and exhaustive external-tool compliance therefore remain unverified when supported only by agent self-report. Requested model/effort controls are recorded; effective backend model identity is not independently attested. These small groups do not establish a performance ranking.
