...
## Repeated model groups and accumulated records

A subsequent cohort used 18 fresh native subagents: requested models `gpt-6-astra`, `gpt-5.6-sol`, and `gpt-5.6-luna`, each at medium effort, with three repetitions of the same experiment and three repetitions of the same literature case. Fixtures and prompts were fixed before execution. An initial CLI bootstrap attempt was rejected by execution policy before it read the skill; it is infrastructure evidence and is excluded from model-quality counts. The native cohort retained the existing tool permissions.

All nine experiment artifacts reported MSE 2.50 versus 10.00, retained the mean predictor, and recorded exactly two launches. Independent runner comparison succeeded for all three Astra-group and all three Sol-group repetitions, and two of three Luna-group repetitions. The remaining repetition included intentionally changed `model.json` in `data_paths`; the runner correctly refused comparability. That report also named absent `run.json` and `execution.log` files. These observations prompted short clarifications about invariant inputs and actual evidence filenames, without changing runner behavior or weakening its checks. Two other repetitions omitted `.gitignore` from declared invariant paths while preserving its bytes; this is recorded separately from the comparability error.

All 90 protected input-file checks across the 18 trials passed. Independent review with the requested-model mapping withheld graded 16 observable artifacts as pass, one as partial, and one as fail. All nine literature artifacts passed. The partial experiment had a valid comparison but cited a missing comparison artifact; it also omitted unchanged README and ignore files from the literal invariant declaration requirement. Complete per-tool transcripts and per-trial token usage are unavailable for these native subagents. Exact skill reads, training-only access chronology, and exhaustive external-tool compliance therefore remain unverified when supported only by agent self-report. Requested model/effort controls are recorded; effective backend model identity is not independently attested. No case received full nine-criterion verification, and these small groups do not establish a performance ranking.
